...
public class CharactersService {
    ...
    public CharactersService(CharacterRepository characterRepository, SessionRepository sessionRepository, ModelMapper mapper, UserRepository userRepository) {
    ...
    public List<CharacterDto> getCharactersByUser(Integer userId) {
        return characterRepository.findByOwner_Id(userId).stream().map(item -> mapper.map(item, CharacterDto.class)).collect(Collectors.toList());
    }

    @Transactional
    public CharacterDto getCharacter(Integer userId, Integer characterId) {
        return mapper.map(characterRepository.findByOwner_IdAndId(userId, characterId), CharacterDto.class);
    }

    @Transactional
    public CharacterDto createCharacter(Integer userId, CreateCharacterPostRequest request){
        Session session = sessionRepository.findByIdAndCreatorId_Id(request.getSessionId(), userId).orElseThrow();
    ...
    @Transactional
    public CharacterDto changeCharacter(Integer userId, Integer characterId, ChangeCharacterPutRequest request){
        Character character = characterRepository.findByOwner_IdAndId(userId, characterId).orElseThrow();
        character.setName(request.getName());
        character.setDescription(request.getDescription());
        characterRepository.save(character);
        return mapper.map(character, CharacterDto.class);
